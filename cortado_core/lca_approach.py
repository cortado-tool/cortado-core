...
from cortado_core.process_tree_utils.miscellaneous import get_number_leaves, get_height, get_number_silent_leaves, \
    get_number_nodes, \
    is_leaf_node, get_index_of_pt_in_children_list
from cortado_core.process_tree_utils.reduction import apply_reduction_rules, reduce_loops_with_more_than_two_children
from cortado_core.process_tree_utils.to_petri_net_transition_bordered import apply as pt_to_petri_net
from cortado_core.utils.alignment_utils import is_log_move, alignment_step_represents_no_deviation, is_sync_move, \
    get_first_deviation, calculate_alignment_typed_trace
from cortado_core.utils.deviation_solvers import DeviationType, get_deviation_solver, Deviation
from cortado_core.utils.start_and_end_activities import \
    ...
    add_artificial_start_end_activity_to_typed_trace, add_artificial_start_end_activity_to_typed_log
...
def add_trace_to_pt_language(pt: ProcessTree, log: Union[EventLog, List[TypedTrace]], trace: Union[Trace, TypedTrace],
                             try_pulling_lca_down=False, add_artificial_start_end=True,
                             pool: Optional[multiprocessing.pool.Pool] = None,
                             only_first_matching_alignment=True) -> ProcessTree:
    """
    ...
    return __add_trace_to_pt_language(pt, log, trace, try_pulling_lca_down=try_pulling_lca_down,
...
def __add_artificial_start_end_activities(pt: ProcessTree, trace: TypedTrace, log: list[TypedTrace],
                                          add_artificial_start_end: bool) -> tuple[
    ProcessTree, TypedTrace, list[TypedTrace], bool]:
    if not add_artificial_start_end and (
            trace.infix_type == InfixType.NOT_AN_INFIX or trace.infix_type == InfixType.PROPER_INFIX):
...
def __add_trace_to_pt_language(pt: ProcessTree, log: Union[EventLog, List[TypedTrace]], trace: TypedTrace,
                               try_pulling_lca_down=False, add_artificial_start_end=True,
                               pool: Optional[multiprocessing.pool.Pool] = None) -> ProcessTree:
    pt, trace, log, art_nodes_added = __add_artificial_start_end_activities(pt, trace, log, add_artificial_start_end)
    ...
    while deviation:
        ...
        if alignment["cost"] >= STD_MODEL_LOG_MOVE_COST:
            # deviation found
            pt = __repair_process_tree(pt, log, alignment, try_pulling_lca_down, pool, trace.infix_type)
        else:
            deviation = False
...
def __repair_process_tree(pt_root: ProcessTree, log: List[TypedTrace], alignment: AlignmentResult,
                          try_pulling_lca_down: bool, pool: Optional[multiprocessing.pool.Pool],
                          infix_type: InfixType) -> ProcessTree:
    logging.debug("repair_process_tree()")

    deviation = __get_deviation(alignment)
    solver = get_deviation_solver(deviation, infix_type, try_pulling_lca_down, pool)
    return solver.solve(deviation, pt_root, log)


def __get_deviation(alignment) -> Deviation:
    step, i = get_first_deviation(alignment)
    if step is None:
        logging.debug("found no deviation")
        return Deviation(DeviationType.NO_DEVIATION, (None, -1), (None, -1), alignment, i)
    ...
    if exec_pt_leave_before_deviation[0] is None and exec_pt_leave_after_deviation[0] is None:
        return Deviation(DeviationType.NOT_ENCLOSED, (None, -1), (None, -1), alignment, i)

    if exec_pt_leave_before_deviation[0] is not None and exec_pt_leave_after_deviation[0] is not None:
        return Deviation(DeviationType.ENCLOSED, exec_pt_leave_before_deviation, exec_pt_leave_after_deviation,
                         alignment, i)

    if exec_pt_leave_before_deviation[0] is not None:
        return Deviation(DeviationType.LEFT_ENCLOSED, exec_pt_leave_before_deviation, (None, -1), alignment, i)

    return Deviation(DeviationType.RIGHT_ENCLOSED, (None, -1), exec_pt_leave_after_deviation, alignment, i)


def __get_leaf_before_deviation(alignment: AlignmentResult, deviation_i: int) -> tuple[ProcessTree, int]:
...
def __get_leaf_after_deviation(alignment: AlignmentResult, deviation_i: int) -> tuple[ProcessTree, int]:
