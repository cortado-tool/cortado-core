...
class TreePattern:
    ...
    def right_most_path_extension(self, pSets: PruningSets):
        """ """
            ...
            while currentNode is not None:
                ...
                if currentNode.op:
                    extended_motifes.extend(
                        extend_node(self, currentNode, extensionOffset, pSets)
                    )
    ...
    def update_rmo_list(
        ...
        supToGain: int,
        repetition_pairs_mining: bool = False,
    ):
        """
        ...
        for oc in self.rmo:
            ...
            for entry in self.rmo[oc]:
                ...
                while offsetHeight != 0:
                    ...
                    if (
                        offsetHeight != 0
                        and eNode.op == cTreeOperator.Sequential
                        and eChild != eNode.children[-1]
                    ):
                        skipped_right = True
                        break
                ...
                # newly added node under the "leaf node"
                    ...
                    if no_seq_predecessor or eNode.op != cTreeOperator.Sequential:
                        for child in eNode.children:
                            if (
                                self.rml.label is not None
                                and child.label == self.rml.label
            ...
            if len(occurences) > 0:
                ...
                cur_sup = _update_current_sup(
                    ...
                    cur_sup,
                    oc,
                    _compute_roots(occurences, unique=not repetition_pairs_mining),
                )
...
def extend_node(
    ...
    if len(eNode.children) > 0:
        if eNode.op == cTreeOperator.Sequential and eNode.rmc.op:
            ...
            closure_labels = closure_labels.intersection(
                pSets.sibPrune[(eNode.op, eNode.rmc.op)]
            )
    ...
    # Operator node doesn't have a Child
        ...
        for l in pSets.nestPrune[(eNode.parent.op, eNode.op)]:
            if isinstance(l, cTreeOperator):
                extended_motifes.append(
                    extend_motif_on_operator_node(tp, eNode, eOffset, op=l, label=None)
                )
            else:
                extended_motifes.append(
                    extend_motif_on_operator_node(tp, eNode, eOffset, op=None, label=l)
                )
