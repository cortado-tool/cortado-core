...
def _computeSeqOperatorClosure(
    ...
    for child in node.children[1:-1]:
        if node.op == cTreeOperator.Concurrent:
            if child.label:
                ...
                labels = labels.intersection(
                    _computeSeqOperatorClosure(child, dfR, efR, rmc)
                )  # RMC remains preserved for
            ...
            if child.label:
                ...
                labels = labels.intersection(
                    _computeSeqOperatorClosure(child, dfR, efR, False)
                )
    ...
    if node.children[-1].label:
        ...
        labels = labels.intersection(
            _computeSeqOperatorClosure(node.children[-1], dfR, efR, rmc)
        )
