...
def create_treebank_from_cv_variants(
    variants: Mapping[Group, List[Trace]], artifical_start=False, add_traces=False
    ...
    for i, group in enumerate(
        sorted(list(variants.keys()), key=lambda x: len(variants[x]), reverse=True)
    ):
