...
from cortado_core.subprocess_discovery.subtree_mining.utilities import _compute_roots
...
def check_min_sup(
    ...
    if freq_strat == FrequencyCountingStrategy.TraceOccurence:
        tp.support = sum(
            [treebank[tid].nTraces * _compute_roots(tp.rmo[tid]) for tid in tp.rmo]
        )
        return tp.support > min_sup
    ...
    if freq_strat == FrequencyCountingStrategy.VariantOccurence:
        tp.support = sum([_compute_roots(tp.rmo[tid], unique_roots) for tid in tp.rmo])
        return tp.support > min_sup
