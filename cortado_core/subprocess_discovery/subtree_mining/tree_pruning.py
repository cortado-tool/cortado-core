...
def _get_prune_sets(fSets: FrequentActivitySets, F) -> PruningSets:
    ...
    operatorOperatorPrune[cTreeOperator.Fallthrough] = set(
        [cTreeOperator.Sequential, cTreeOperator.Concurrent, cTreeOperator.Fallthrough]
    )
    operatorOperatorPrune[cTreeOperator.Concurrent] = set(
        [cTreeOperator.Concurrent, cTreeOperator.Fallthrough]
    )
