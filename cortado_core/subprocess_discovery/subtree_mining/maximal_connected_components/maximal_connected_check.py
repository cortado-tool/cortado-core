...
def check_maximality_closed_patterns(tp, k, level_to_go):
    ...
    if isinstance(tp, CMTreePattern):
        eSubtrees += [
            (k - 1, sub) for sub in _compute_left_most_path_eliminated_leafs(tp.tree)
        ]
        eSubtrees += [
            (k - 1, sub) for sub in _compute_right_most_path_eliminated_leafs(tp.tree)
        ]

    else:
        eSubtrees += [
            (k - 1, sub) for sub in compute_valid_leaf_eliminated_children(tp.tree)
        ]
        eSubtrees += [
            (k - 1, sub) for sub in _compute_left_most_path_eliminated_leafs(tp.tree)
        ]
