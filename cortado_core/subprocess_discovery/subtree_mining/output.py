...
from cortado_core.subprocess_discovery.subtree_mining.metrics.confidence import (
    child_parent_confidence,
    cross_support_confidence,
...
def add_confidence_information_to_df(k_pattern, df):
    ...
    df["subpattern_confidence"] = df.obj.apply(
        lambda x: subpattern_confidence(x, tp_sup_dict)
    )
    df["cross_support_confidence"] = df.obj.apply(
        lambda x: cross_support_confidence(x, tp_sup_dict)
    )
