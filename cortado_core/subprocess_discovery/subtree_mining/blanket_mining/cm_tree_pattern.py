...
class CMTreePattern(TreePattern):
    def __init__(self, T: CMConcurrencyTree, rml: CMConcurrencyTree, heightDiff: int):
        ...
        super().__init__(T, rml, heightDiff)
        self.tree: CMConcurrencyTree = T
        self.parent: CMTreePattern | None = None
        self.support: int | None = None
        self.maximal: bool = False
        self.closed: bool = False
        self.size: int | None = None
        self.rml: CMConcurrencyTree = rml
        self.heightDiff: int = heightDiff
        self.rightBlanket: CMConcurrencyTree | None = None
        ...
        for oc in self.rmo:
            ...
            if len(occurences) > 0:
                ...
                cur_sup = _update_current_sup(
                    treeBank, freq_strat, cur_sup, oc, _compute_roots(occurences)
        ...
        for occ in tree.occList:
            if occ in maskMap:
                occList[occ] = flatten(
                    [
                        list(repeat(x, y))
                        for x, y in zip(tree.occList[occ], maskMap[occ])
                    ]
                )
