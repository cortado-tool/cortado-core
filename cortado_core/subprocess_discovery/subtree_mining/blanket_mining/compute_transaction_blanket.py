...
def check_concurrent_transaction_blanket_match(
    node: CMConcurrencyTree, sibLabels: Set[str], expDir: ExpansionDirection
):
...
def check_seq_transaction_blanket_match(
    node: CMConcurrencyTree, sibLabels: Set[str], expDir: ExpansionDirection
):
    if expDir == ExpansionDirection.Left:
        lChild = node.children[0]
...
def compute_blanket_transactions_candidates(
    tp: CMTreePattern, tree: CMConcurrencyTree, tid: int
):
        ...
        if len(tree.children) > 0:
            ...
            for idx, occurence in enumerate(occurences):
                ...
                if lidx != ridx:
                    ...
                    betweenSiblings = get_child_labels(
                        occurence.children[lidx + 1 : ridx]
                    )
