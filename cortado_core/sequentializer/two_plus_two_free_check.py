...
def can_apply_without_violating_two_plus_two_freeness(
...
def contains_possibly_violating_construct(target_pattern: SequentializerPattern):
    wildcard_node = get_wildcard_node(target_pattern)
...
def node_is_parallel_to_sequence(node: SequentializerPattern):
    parent = node.parent
    if parent is None:
        return False
...
def get_wildcard_node(
    pattern: SequentializerPattern,
) -> Optional[SequentializerPattern]:
