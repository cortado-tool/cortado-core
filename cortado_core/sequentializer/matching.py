...
from cortado_core.sequentializer.pattern import SequentializerPattern, WILDCARD_MATCH


def is_matching(tree: ConcurrencyTree, source_pattern: SequentializerPattern):
    if tree.op is None or (
        tree.op != cTreeOperator.Concurrent and tree.op != cTreeOperator.Fallthrough
    ):
        return False, None

    wildcard_child = None
    tree_children = [
        child for child in tree.children if child.op != cTreeOperator.Fallthrough
    ]
...
def get_target_to_source_mapping(
    ...
    for source_node in source_nodes:
        ...
        target_nodes_dict[tuple(source_node.labels)] = target_nodes_dict[
            tuple(source_node.labels)
        ][1:]
...
def get_non_operator_nodes(pattern: SequentializerPattern):
    nodes = []
...
def get_first_match(
    label: str, leaves: List[ConcurrencyTree]
