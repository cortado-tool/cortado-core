...
from typing import List, Tuple, Dict, OrderedDict, FrozenSet, Iterable, Union
...
def add_trace_to_pt_language_with_freezing(
    pt: ProcessTree,
    ...
    :param log: EventLog or list of typed traces, accepted by pt
    ...
    if not typed_trace_fits_process_tree(trace, pt):
        ...
        # if whole tree is frozen add parallel node as parent with tau as sibling
        if all([children in frozen_subtrees for children in pt.children]):
            frozen_subtrees = [pt]
            pt = ProcessTree(operator=Operator.PARALLEL, children=[pt, ProcessTree()])
            for children in pt.children:
                children.parent = pt
        ...
        incremental_projected_logs, final_projected_log = project_log(
            pt, log, frozen_subtrees_replacement_label
        )

        incremental_projected_traces: Dict[FrozenSet[Tuple[ProcessTree, int]], Trace]
        final_projected_trace: Trace
        incremental_projected_traces, final_projected_trace = project_trace(
            trace, frozen_subtrees_replacement_label
        )
        ...
        pt = add_trace_to_pt_language(
            pt,
            final_projected_log,
            final_projected_trace,
            try_pulling_lca_down=try_pulling_lca_down,
            add_artificial_start_end=add_artificial_start_end,
            pool=pool,
        )
        assert typed_trace_fits_process_tree(final_projected_trace, pt)
        res_print = []
        for a in final_projected_trace.trace:
        ...
        for k in frozen_subtrees_replacement_label:
            assert (
                frozen_subtrees_replacement_label[k] not in label_to_frozen_subtree
            )  # labels are unique
            label_to_frozen_subtree[frozen_subtrees_replacement_label[k]] = k[0]
        pt = reinsert_frozen_subtrees(
            label_to_frozen_subtree,
            pt,
            incremental_projected_logs,
            log,
            incremental_projected_traces,
            trace,
        ...
        assert typed_trace_fits_process_tree(trace, pt)
...
if __name__ == "__main__":
    ...
    print(frozen_trees)
    res, frozen_subtrees = add_trace_to_pt_language_with_freezing(
