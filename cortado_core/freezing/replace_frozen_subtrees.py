...
def replace_frozen_subtrees_in_pt(
    pt: ProcessTree, subtrees_to_be_replaced: Dict[Tuple[ProcessTree, int], str]
) -> ProcessTree:
    for subtree, subtree_id in subtrees_to_be_replaced:
        pt = __replace_subtree_in_pt(
            pt, subtree, subtrees_to_be_replaced[(subtree, subtree_id)]
        )
    return pt


def __replace_subtree_in_pt(
    pt: ProcessTree, subtree_to_be_replaced: ProcessTree, replacement_label: str
) -> ProcessTree:
    if subtree_is_part_of_tree_based_on_obj_id(subtree_to_be_replaced, pt):
        ...
        replacement_tree: ProcessTree = __generate_replacement_tree_from_label(
            replacement_label
        )
...
if __name__ == "__main__":
    ...
    print(
        subtree_is_part_of_tree_based_on_obj_id(pt_parse("->('A','F') )"), pt_1)
    )  # True
    ...
    subtrees = {
        pt_dict_key(pt_1.children[1].children[1]): "X_1",
