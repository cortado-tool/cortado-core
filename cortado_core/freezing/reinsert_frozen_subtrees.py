...
def reinsert_frozen_subtrees(
    subtrees_to_insert: OrderedDict[str, ProcessTree],
    pt: ProcessTree,
    ...
    for replacement_label, frozen_subtree in subtrees_to_insert.items():
        ...
        if len(frozen_subtrees_to_be_inserted) > 0:
            projected_log = incremental_projected_logs[
                frozenset(frozen_subtrees_to_be_inserted)
            ]
            projected_trace = incremental_projected_traces[
                frozenset(frozen_subtrees_to_be_inserted)
            ]
        else:
        ...
        pt_activated: List[ProcessTree] = __get_leaf_node_by_label(
            pt, replacement_label + "+ACTIVATED"
        )
        pt_closed: List[ProcessTree] = __get_leaf_node_by_label(
            pt, replacement_label + "+CLOSED"
        )
        ...
        if len(pt_activated) > 1:
            for i in range(1, len(pt_activated)):
                tree_changed = True
                while tree_changed:
                    res, tree_changed = find_lowest_common_ancestor(
                        lca_pt_activated, pt_activated[i], True
                    )
        ...
        if len(pt_closed) > 1:
            for i in range(1, len(pt_closed)):
                ...
                while tree_changed:
                    res, tree_changed = find_lowest_common_ancestor(
                        lca_pt_closed, pt_closed[i], True
                    )
                lca_pt_closed = res
        ...
        tree_changed = True
        while tree_changed:
            lca, tree_changed = find_lowest_common_ancestor(
                lca_pt_activated, lca_pt_closed, True
            )
        ...
        if (
            ...
            and lca.children[0] is lca_pt_activated
            and lca.children[1] is lca_pt_closed
            and is_leaf_node(lca_pt_closed)
            and is_leaf_node(lca_pt_activated)
        ):
            # standard case ==> replace ->('replacement_label+ACTIVATED', 'replacement_label+CLOSED') by frozen tree
            assert lca.parent
            replace_tree_in_children(
                lca.parent, lca, subtrees_to_insert[replacement_label]
            )
        else:
            ...
            while not appropriate_insert_position_found:
                ...
                ACTIVATED_execution_numbers = calculate_execution_numbers(
                    insert_candidate, replacement_label + "+ACTIVATED"
                )
                CLOSED_execution_numbers = calculate_execution_numbers(
                    insert_candidate, replacement_label + "+CLOSED"
                )
                intersection = ACTIVATED_execution_numbers.intersection(
                    CLOSED_execution_numbers
                )

                logging.debug(
                    "ACTIVATED_execution_numbers", ACTIVATED_execution_numbers
                )
                ...
                assert (
                    intersection == {0, 1}
                    or intersection == {1}
                    or intersection == {1, math.inf}
                    or intersection == {0, 1, math.inf}
                )

                inserted_parallel: ProcessTree = None
                if intersection == {1}:
                    inserted_parallel = apply_case_1(
                        insert_candidate, subtrees_to_insert[replacement_label]
                    )
                elif intersection == {0, 1}:
                    inserted_parallel = apply_case_0_1(
                        insert_candidate, subtrees_to_insert[replacement_label]
                    )
                elif intersection == {1, math.inf}:
                    inserted_parallel = apply_case_1_inf(
                        insert_candidate, subtrees_to_insert[replacement_label]
                    )
                elif intersection == {0, 1, math.inf}:
                    inserted_parallel = apply_case_0_1_inf(
                        insert_candidate, subtrees_to_insert[replacement_label]
                    )
                ...
                # tree_vis.view(tree_vis.apply(pt, parameters={"format": "svg"}))
                if typed_trace_fits_process_tree(projected_trace, pt):
                    for trace in projected_log:
                        if not typed_trace_fits_process_tree(trace, pt):
                ...
                if not appropriate_insert_position_found:
                    ...
                    replace_tree_in_children(
                        inserted_parallel.parent, inserted_parallel, insert_candidate
                    )
    ...
    frozen_trees: List[ProcessTree] = [
        subtrees_to_insert[k] for k in subtrees_to_insert
    ]
...
def post_process_tree(pt: ProcessTree, excluded_subtrees=[]) -> ProcessTree:
    ...
    while tree_changed:
        ...
        pt = remove_operator_node_with_one_or_no_child(
...
def add_missing_frozen_subtrees(
    pt: ProcessTree, missing_frozen_subtrees: List[ProcessTree]
) -> ProcessTree:
    ...
    for missing_tree in missing_frozen_subtrees:
        inner_xor_node = ProcessTree(
            parent=new_root_node, operator=Operator.XOR, children=[]
        )
...
def replace_activation_and_closing_by_tau(
    tree: ProcessTree, label_to_be_removed: str
) -> Dict[Tuple[ProcessTree, int], str]:
    leaves_activated = replace_given_leaf_node_label_by_tau(
        tree, label_to_be_removed + "+ACTIVATED"
    )
    leaves_closed = replace_given_leaf_node_label_by_tau(
        tree, label_to_be_removed + "+CLOSED"
    )
    return {**leaves_activated, **leaves_closed}


def apply_case_1(lca: ProcessTree, frozen: ProcessTree) -> ProcessTree:
    parallel = ProcessTree(
        operator=Operator.PARALLEL, parent=lca.parent, children=[lca, frozen]
    )
    if lca.parent:
...
def apply_case_0_1(lca: ProcessTree, frozen: ProcessTree) -> ProcessTree:
    ...
    parallel = ProcessTree(
        operator=Operator.PARALLEL, parent=lca.parent, children=[lca, choice]
    )
...
def apply_case_1_inf(lca: ProcessTree, frozen: ProcessTree) -> ProcessTree:
    ...
    frozen.parent = loop
    parallel = ProcessTree(
        operator=Operator.PARALLEL, parent=lca.parent, children=[lca, loop]
    )
...
def __get_leaf_node_by_label(
    tree: ProcessTree, label: str, res: List[ProcessTree] = None
) -> List[ProcessTree]:
    if res is None:
        res = []
    if tree.label == label:
...
def replace_given_leaf_node_label_by_tau(
    pt: ProcessTree, label: str, replacement_log=None
) -> Dict[Tuple[ProcessTree, int], str]:
    ...
    if pt.label and pt.label == label:
        ...
        for c in pt.children:
            replacement_log = replace_given_leaf_node_label_by_tau(
                c, label, replacement_log
            )
    return replacement_log


def undo_replace_leaves_with_label_by_tau(
    pt: ProcessTree, replacement_log: Dict[Tuple[ProcessTree, int], str]
):
