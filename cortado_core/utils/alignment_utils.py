...
from cortado_core.process_tree_utils.to_petri_net_transition_bordered import (
    apply as pt_to_petri_net_cortado,
)
...
def alignment_step_represents_no_deviation(alignment_step) -> bool:
    return not is_log_move(alignment_step) and not is_model_move_on_visible_activity(
        alignment_step
    )
...
def is_sync_move(alignment_step) -> bool:
    return (
        alignment_step_represents_no_deviation(alignment_step)
        and alignment_step[1][1] is not None
    )
...
def calculate_alignment_typed_trace(pt: ProcessTree, trace: TypedTrace):
    match trace.infix_type:
        ...
        case inf_type:
            return calculate_infix_postfix_prefix_alignment(
                trace.trace, pt, inf_type, copy_tree=False
            )
...
def calculate_infix_postfix_prefix_alignment(
    trace: Trace, pt: ProcessTree, infix_type: InfixType, copy_tree=True
):
    ...
    match infix_type:
        case InfixType.PROPER_INFIX:
            alignment = infix_alignments.calculate_optimal_infix_alignment(
                trace,
                pt,
        ...
        case InfixType.PREFIX:
            alignment = prefix_alignments.calculate_optimal_prefix_alignment(
                trace,
                pt,
                use_dijkstra=True,
                use_cortado_tree_converter=True,
        ...
        case InfixType.POSTFIX:
            alignment = suffix_alignments.calculate_optimal_suffix_alignment(
                trace,
                pt,
                naive=False,
                use_dijkstra=True,
