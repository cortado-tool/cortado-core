...
from cortado_core.trace_ordering.strategy_component.strategy_component import (
    StrategyComponent,
)
...
class Strategy:
    ...
    def apply(
        ...
        for sc, candidates_filter in self.components_filter:
            candidates = sc.apply(
                log, previously_added_variants, process_tree, candidates
            )
    ...
    def apply_trace(
        ...
        process_tree: ProcessTree,
        trace_candidates: List[Trace],
    ) -> Trace:
        previously_added_variants = [
            self.__trace_to_sequence_group(t) for t in previously_added_traces
        ]
        variant_candidates = [
            self.__trace_to_sequence_group(t) for t in trace_candidates
        ]

        selected_variant = self.apply(
            log, previously_added_variants, process_tree, variant_candidates
        )
