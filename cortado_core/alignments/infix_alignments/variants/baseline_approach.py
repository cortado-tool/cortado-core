...
def calculate_optimal_infix_alignment(
    ...
    params = {prefix_alignments.Parameters.PARAM_MAX_ALIGN_TIME_TRACE: timeout}
...
def build_extended_petri_net_for_infix_alignments(
    ...
    reachable_markings, is_timeout = infix_utils.generate_reachable_markings(
        net, im, timeout
    )
    if is_timeout:
        raise TimeoutError()
    ...
    if not naive:
        ...
        reachable_markings = __filter_reachable_markings(
            net, fm, reachable_markings, trace_activities
        )

    net, new_im, added_transitions = __add_invisible_tau_transitions(
        net, reachable_markings
    )
...
def __filter_reachable_markings(
    net: PetriNet,
    fm: Marking,
    reachable_markings: Set[Marking],
...
def __add_invisible_tau_transitions(
    net: PetriNet, reachable_markings: Set[Marking]
) -> Tuple[PetriNet, Marking, list]:
