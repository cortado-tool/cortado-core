...
def __repair_model_move(step, process_tree_root):
    ...
    index_last_executed_leave_node = get_index_of_pt_in_children_list(
        model_move_leave.parent, model_move_leave
    )
...
def __repair_log_move(log_move_activity, last_executed_leave_node, process_tree_root):
    ...
    if last_executed_leave_node:
        ...
        sequence_pt = ProcessTree(
            operator=Operator.SEQUENCE,
            parent=last_executed_leave_node.parent,
        ...
        if sequence_parent is None:
            ...
            index_last_executed_leave_node = get_index_of_pt_in_children_list(
                sequence_parent, last_executed_leave_node
            )
        ...
        sequence_pt = ProcessTree(
            operator=Operator.SEQUENCE, children=[choice_pt, process_tree_root]
        )
...
def repair_first_deviation(alignment, process_tree_root, debug=DEBUG):
    ...
    for i, step in enumerate(alignment["alignment"]):
        ...
        is_model_move_on_pt = (
            step[1][0] == SKIP
            and isinstance(step[0][1][0], ProcessTree)
            and is_leaf_node(step[0][1][0])
            and step[0][1][0].label
        )
        ...
        if is_model_move_on_pt:
            ...
            res: ProcessTree = __repair_log_move(
                log_move_activity, last_executed_leave_node, process_tree_root
            )
            apply_reduction_rules(res)
            return res


def ensure_trace_replayable_on_process_tree(
    trace: Trace, process_tree_root: ProcessTree, debug=DEBUG
):
    ...
    while deviation_found:
        ...
        if deviation_found:
            ...
            if debug:
                tree_vis.view(
                    tree_vis.apply(
                        process_tree_root, parameters={"format": "svg", "debug": True}
                    )
                )
    return process_tree_root
