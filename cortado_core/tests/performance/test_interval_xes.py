...
class TestPerformanceIntervalXes(unittest.TestCase):
    ...
    def test_performance_interval(self):
            ...
            [
                Trace(
                    [
                        Event(e)
                        for e in t
                        if e[DEFAULT_TRANSITION_KEY].lower() in lifecycles
                    ]
                )
                for t in event_log
            ]
        ...
        total_order_variants = [
            tv for v in variants_sorted[:5] for tv in generate_variants(v[0])
        ]
        train_log = [
            Trace([Event({DEFAULT_NAME_KEY: a}) for a in v])
            for v in total_order_variants
        ]
        train_log = EventLog(train_log)

        test_log = EventLog(
            [Trace(t) for v, traces in variants_sorted[:10] for t in traces]
        )
        ...
        (
            ...
            all_waiting_times,
            all_cycle_times,
        ), mean_fitness = tree_performance.get_tree_performance_intervals(pt, test_log)
