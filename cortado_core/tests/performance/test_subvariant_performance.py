...
class TestSubvariantPerformance(unittest.TestCase):
    @staticmethod
    def __create_event(
        name, start_timestamp, complete_timestamp, use_lifecycle: bool = False
    ):
        event = Event()
    ...
    def test_basic(self):
        events = []
        events += self.__create_event(
            "a", datetime(2020, 1, 1, 1), datetime(2020, 1, 1, 2)
        )
        events += self.__create_event(
            "b", datetime(2020, 1, 1, 3), datetime(2020, 1, 1, 4)
        )
        ...
        subvariant_with_performance = (
            subvariant_performance.calculate_subvariant_performance(
                subvariant, [trace], TimeUnit.SEC
            )
        )

        self.assertEqual(
            subvariant_with_performance.global_performance_stats["mean"], 10800
        )
        ...
        self.assertEqual(
            waiting_time_event.start, SubvariantNode("a", "complete", 0, None)
        )
        self.assertEqual(
            waiting_time_event.complete, SubvariantNode("b", "start", 0, None)
        )

    def test_basic_multiple_traces(self):
        traces = []
        events1 = []
        events1 += self.__create_event(
            "a", datetime(2020, 1, 1, 1), datetime(2020, 1, 1, 2)
        )
        events1 += self.__create_event(
            "b", datetime(2020, 1, 1, 3), datetime(2020, 1, 1, 4)
        ...
        events2 = []
        events2 += self.__create_event(
            "a", datetime(2020, 1, 1, 1), datetime(2020, 1, 1, 3)
        )
        events2 += self.__create_event(
            "b", datetime(2020, 1, 1, 4), datetime(2020, 1, 1, 5)
        )
        ...
        subvariant_with_performance = (
            subvariant_performance.calculate_subvariant_performance(
                subvariant, traces, TimeUnit.SEC
            )
        )
    ...
    def test_basic_multiple_traces_with_repeating_activity(self):
        ...
        events1 += self.__create_event(
            "a", datetime(2020, 1, 1, 5), datetime(2020, 1, 1, 6)
        )
        ...
        events2 += self.__create_event(
            "a", datetime(2020, 1, 1, 6), datetime(2020, 1, 1, 7)
        )
        ...
        for parallel_subvariant_nodes in subvariant_with_performance.subvariant:
            for subvariant_node in parallel_subvariant_nodes:
                ...
                if (
                    subvariant_node.activity == "a"
                    and subvariant_node.activity_instance == 0
                ):
                    self.assertEqual(subvariant_node.performance_stats["mean"], 5400)
                    self.assertEqual(subvariant_node.performance_stats["min"], 3600)
                    self.assertEqual(subvariant_node.performance_stats["max"], 7200)
    ...
    def test_parallel_events(self):
        ...
        events += self.__create_event(
            "b", datetime(2020, 1, 1, 1), datetime(2020, 1, 1, 2)
        )
    ...
    def test_overlapping_events(self):
        ...
        events += self.__create_event(
            "b", datetime(2020, 1, 1, 2), datetime(2020, 1, 1, 4)
        )
