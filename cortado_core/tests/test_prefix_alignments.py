...
class TestInfixAlignments(unittest.TestCase):
    def __test_tree(self, tree, acceptable, unacceptable):
        ...
        for variant in self.__get_variants():
            for prefix in acceptable:
                ...
                self.assertLess(
                    alignment["cost"], align_utils.STD_MODEL_LOG_MOVE_COST, prefix
                )
    ...
    def __get_variants(self):
        variant_dijkstra = lambda t, pn, im, fm: prefix_alignments.apply_trace(
            t, pn, im, fm, variant=prefix_alignments.VERSION_DIJKSTRA_NO_HEURISTICS
        )
        variant_a_star = lambda t, pn, im, fm: prefix_alignments.apply_trace(
            t, pn, im, fm, variant=prefix_alignments.VERSION_A_STAR
        )
