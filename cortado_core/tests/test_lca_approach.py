...
class LCA_Approach_Tests(unittest.TestCase):
    ...
    def test_lca_approach_add_infix(self):
        ...
        previously_added = [TypedTrace(generate_test_trace('abcd'), InfixType.NOT_AN_INFIX),
                            TypedTrace(generate_test_trace('abdc'), InfixType.NOT_AN_INFIX)]

        expected_tree = pt_parse("->('a', 'b', X(tau, 'e'), +('c', 'd'))")
        new_tree = add_trace_to_pt_language(tree, previously_added, trace_to_add, try_pulling_lca_down=True,
                                            add_artificial_start_end=False)
    ...
    def test_infixes_in_already_added_traces(self):
        ...
        previously_added = [TypedTrace(generate_test_trace('a'), InfixType.PREFIX),
    ...
    def test_add_infix_without_lca_pulldown(self):
        ...
        previously_added = [TypedTrace(generate_test_trace('abcdef'), InfixType.NOT_AN_INFIX)]
        expected_tree = pt_parse("->( 'a', 'b', 'c', X( tau, 'd' ), 'e', 'f' )")
        new_tree = add_trace_to_pt_language(tree, previously_added, trace_to_add, try_pulling_lca_down=False,
                                            add_artificial_start_end=True)
        self.assertEqual(tree_sort(expected_tree), tree_sort(new_tree))
    ...
    def test_random_traces(self):
        ...
        tree = pt_parse("->('a', 'b', +('c', 'd'))")
        log = [generate_random_trace(8, ['a', 'b', 'c', 'd', 'e', 'f']) for _ in range(10)]
        added = []

        for idx, trace_to_add in enumerate(log):
            tree = add_trace_to_pt_language(tree, added, trace_to_add, try_pulling_lca_down=True,
                                            add_artificial_start_end=False)
