...
from cortado_core.sequentializer.algorithm import (
    apply_sequentializer,
    apply_sequentializer_on_variants,
)
from cortado_core.sequentializer.pattern import parse_sequentializer_pattern


class TestSequentializerAlgorithm(unittest.TestCase):
    def test_algorithm_specific_match(self):
        variant = parse_concurrency_tree("+('b','c')")
        source_pattern = parse_sequentializer_pattern("+('b','c')")
        target_pattern = parse_sequentializer_pattern("->('b','c')")

        new_variant = apply_sequentializer(variant, source_pattern, target_pattern)

        self.assertEqual("→(b, c)", str(new_variant))

    def test_algorithm_specific_match_on_lower_level_1(self):
        variant = parse_concurrency_tree(
            "->('a', 'b', +('b','c', ->('d', +('b','c'))))"
        )
        source_pattern = parse_sequentializer_pattern("+('b','c')")
        target_pattern = parse_sequentializer_pattern("->('b','c')")
    ...
    def test_algorithm_no_sub_specific_match(self):
        variant = parse_concurrency_tree("->('a', 'b', +('b','c','d'))")
        source_pattern = parse_sequentializer_pattern("+('b','c', 'e' ...)")
        target_pattern = parse_sequentializer_pattern("->('b','c', 'e', ...)")
    ...
    def test_algorithm_specific_match_set_notation(self):
        ...
        source_pattern = parse_sequentializer_pattern("+({'a','c'}, {'b'})")
        target_pattern = parse_sequentializer_pattern("->({'a','c'}, {'b'})")
    ...
    def test_algorithm_fallthrough_set_notation_match(self):
        ...
        source_pattern = parse_sequentializer_pattern("+({'a','c'}, {'b','d'})")
        target_pattern = parse_sequentializer_pattern("->({'a','c'}, {'b','d'})")
    ...
    def test_apply_on_variants(self):
        ...
        v3 = parse_concurrency_tree(
            "->('a', 'b', 'e', 'c', 'd')"
        ).to_concurrency_group()
        v4 = parse_concurrency_tree(
            "->('a', 'b', ->('e', 'c', 'd'))"
        ).to_concurrency_group()
        trace = Trace([])
        ...
        source_pattern = parse_sequentializer_pattern("+('a', 'b', ...)")
        target_pattern = parse_sequentializer_pattern("->('a', 'b',...)")

        new_variants = apply_sequentializer_on_variants(
            variants, source_pattern, target_pattern
        )
