...
class TestQueryEval(unittest.TestCase):
    def test_evaluate_leafs_non_quantified(self):
        query_pairs = {
            ...
            "('A' isC -> 'A' isDF 'B') OR 'I' isP ALL {'A','B', 'C'};": [
                0,
                1,
                3,
                4,
            ...
            "('A' isC OR 'H' isC) -> ('A' isDF 'B' OR 'H' isDF 'B');": [
                0,
                1,
                3,
                4,
    ...
    def test_evaluate_self_concurrency(self):
        log = EventLog()
        # *-A-*
        #       *--B--*
        #        *-B-*
        ...
        e1[DEFAULT_START_TIMESTAMP_KEY] = datetime.now() + timedelta(hours=0)
        ...
        e2[DEFAULT_TIMESTAMP_KEY] = datetime.now() + timedelta(hours=5)

        e3[DEFAULT_NAME_KEY] = "B"
        e3[DEFAULT_START_TIMESTAMP_KEY] = datetime.now() + timedelta(hours=3)
        e3[DEFAULT_TIMESTAMP_KEY] = datetime.now() + timedelta(hours=4)
        ...
        pt = parse_query_to_tree("'B' isP 'B';")
        qt = convertParseTreeToQueryTree(pt)

        assert check_query_tree(qt, list(variant.graphs.keys())[0], activities)
