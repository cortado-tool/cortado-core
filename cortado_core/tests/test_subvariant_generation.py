...
class TestSubvariantGeneration(unittest.TestCase):
    ...
    def test_parallel_starting_timestamps(self):
        ...
        events = []
        events += self.__create_event(
            "a", datetime(2020, 1, 1, 2), datetime(2020, 1, 1, 4)
        )
        events += self.__create_event(
            "a", datetime(2020, 1, 1, 2), datetime(2020, 1, 1, 3)
        )
        events += self.__create_event(
            "b", datetime(2020, 1, 1, 7), datetime(2020, 1, 1, 9)
        )
        ...
        expected = (
            (
                SubvariantNode("a", "start", 0, None),
                SubvariantNode("a", "start", 1, None),
            ),
            (SubvariantNode("a", "complete", 0, None),),
            (SubvariantNode("a", "complete", 1, None),),
            (SubvariantNode("b", "start", 0, None),),
