...
class FrequencyCounting(unittest.TestCase):
    ...
    def test_count_frequencies_trace_occurence(self):
        ...
        frequentActivitySet: FrequentActivitySets = ct_compute_frequent_activity_sets(
            treebank, strat, min_sup=0
        )

        self.assertTrue(set(frequentActivitySet.dfR["A"]) == set(["B", "D", "A", "C"]))
        self.assertTrue(
            set(frequentActivitySet.ccR.keys()) == set(["A", "D", "I", "B", "G", "C"])
        )
        self.assertTrue("I" in frequentActivitySet.ccR["B"])
        self.assertTrue(
            set(frequentActivitySet.dfR.keys()) == set(["A", "B", "C", "G", "H"])
        )
        ...
        frequentActivitySet: FrequentActivitySets = ct_compute_frequent_activity_sets(
            treebank, strat, min_sup=5
        )

        self.assertTrue(
            set(frequentActivitySet.dfR["A"]) == set(["B"])
            and set(frequentActivitySet.dfR["B"]) == set(["C"])
        )
...
class FrequencyCountingInitialCandidatesOfSizeThree(unittest.TestCase):
    def test_count_frequencies_trace_occurence(self):
        ...
        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=0)

        self.assertTrue(set(frequentActivitySet.dfR["A"]) == set(["B", "D", "A", "C"]))
        self.assertTrue(
            set(frequentActivitySet.dfR.keys()) == set(["A", "B", "C", "G", "H"])
        )
        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=5)
    ...
    def test_count_frequencies_trace_transaction(self):
        ...
        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=3)
    ...
    def test_count_frequencies_variant_transaction(self):
        ...
        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=2)

        self.assertTrue(set(frequentActivitySet.dfR["A"]) == set(["B", "C"]))
        self.assertTrue(set(frequentActivitySet.dfR["B"]) == set("C"))
        self.assertTrue("H" in frequentActivitySet.dfR)  # Appears in 3 variants

        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=7)
    ...
    def test_count_frequencies_variant_occurence(self):
        ...
        frequentActivitySet, _, _ = compute_freq3(treebank, strat, min_sup=6)
