...
class TreeBank(unittest.TestCase):
    ...
    def test_generate_art_start_treebank(self):
        ...
        self.assertTrue(len(list(treebank)) == 10)
        self.assertTrue(
            treebank_dict["→(CT_ARTIFICAL_START_NAME, A, B, CT_ARTIFICAL_END_NAME)"]
            == 2
        )
        self.assertTrue(
            treebank_dict[
                "→(CT_ARTIFICAL_START_NAME, H, ∧(G, →(B, C)), CT_ARTIFICAL_END_NAME)"
            ]
            == 2
        )
        self.assertTrue(
            treebank_dict[
                "→(CT_ARTIFICAL_START_NAME, H, ∧(G, →(B, C)), K, CT_ARTIFICAL_END_NAME)"
            ]
            == 1
        )
        self.assertTrue(
            treebank_dict[
                "→(CT_ARTIFICAL_START_NAME, A, B, A, B, CT_ARTIFICAL_END_NAME)"
            ]
            == 1
        )
        self.assertTrue(
            treebank_dict[
                "→(CT_ARTIFICAL_START_NAME, ∧(I, →(A, ∧(B, C))), CT_ARTIFICAL_END_NAME)"
            ]
            == 1
        )
        self.assertTrue(
            treebank_dict[
                "→(CT_ARTIFICAL_START_NAME, A, ∧(B, C, D), CT_ARTIFICAL_END_NAME)"
            ]
            == 1
        )
