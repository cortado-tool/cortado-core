...
class BlanketMining(unittest.TestCase):
    @unittest.skip("does not run in pipeline, TODO fix")
    def test_TraceTransaction_Mining(self):
        ...
        self.assertIn(
            "∧(G, →( )) with support: 6", patterns_3, "∧(G, →( )) with support: 6"
        )
        ...
        self.assertNotIn(
            "∧(→(B, C)) with support: 6", patterns_5, "∧(→(B, C)) with support: 6"
        )
        self.assertNotIn(
            "→(H, ∧(→(B))) with support: 6", patterns_5, "→(H, ∧(→(B))) with support: 6"
        )
    ...
    def test_VariantTransaction_Mining(self):
        ...
        self.assertIn(
            "→(∧(B, C)) with support: 3", patterns_4, "→(B, C) with support: 6"
        )

        patterns_5 = [str(x) for x in k_patterns[5]]

        self.assertIn(
            "→(A, ∧(B, C)) with support: 3", patterns_5, "→(A, ∧(B, C)) with support: 3"
        )

        self.assertNotIn(
            "∧(→(B, C)) with support: 4", patterns_5, "∧(→(B, C)) with support: 4"
        )
        self.assertNotIn(
            "→(H, ∧(→(B))) with support: 4", patterns_5, "→(H, ∧(→(B))) with support: 4"
        )

        patterns_6 = [str(x) for x in k_patterns[6]]

        self.assertNotIn(
            "→(H, ∧(→(B, C))) with support: 4",
            patterns_6,
            "→(H, ∧(→(B, C))) with support: 4",
        )
    ...
    def test_VariantOccurence_Mining(self):
        ...
        self.assertIn(
            "→(∧(B, C)) with support: 3", patterns_4, "→(B, C) with support: 3"
        )
