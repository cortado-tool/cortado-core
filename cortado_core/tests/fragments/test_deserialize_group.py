...
class DeserializeGroupTestCase(unittest.TestCase):
    def test_deserialize_parallel_group(self):
        variant_serialized = {"parallel": [{"leaf": ["a"]}, {"leaf": ["b"]}]}

        result = Group.deserialize(variant_serialized)

        expected = ParallelGroup(lst=[LeafGroup(lst=["a"]), LeafGroup(lst=["b"])])

        self.assertEqual(result, expected)

    def test_deserialize_sequence_group(self):
        variant_serialized = {"follows": [{"leaf": ["a"]}, {"leaf": ["b"]}]}
        result = Group.deserialize(variant_serialized)

        expected = SequenceGroup(lst=[LeafGroup(lst=["a"]), LeafGroup(lst=["b"])])
