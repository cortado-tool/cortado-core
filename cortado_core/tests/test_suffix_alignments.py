...
from cortado_core.alignments.suffix_alignments.algorithm import (
    calculate_optimal_suffix_alignment,
)
...
class TestSuffixAlignments(unittest.TestCase):
    def __test_tree(self, tree, acceptable_suffixes, unacceptable_suffixes):
        ...
        for variant in self.__get_variants():
            for suffix in acceptable_suffixes:
                trace = generate_test_trace(suffix)
                alignment = variant(trace, process_tree)
                # less than because tau moves have a small cost, too
                self.assertLess(
                    alignment["cost"], align_utils.STD_MODEL_LOG_MOVE_COST, suffix
                )
    ...
    def __get_variants(self):
        variant_dijkstra_naive = lambda t, pt: calculate_optimal_suffix_alignment(
            t, pt, naive=True, use_dijkstra=True
        )
        variant_dijkstra_not_naive = lambda t, pt: calculate_optimal_suffix_alignment(
            t, pt, naive=False, use_dijkstra=True
        )
        variant_a_star_naive = lambda t, pt: calculate_optimal_suffix_alignment(
            t, pt, naive=True, use_dijkstra=False
        )
        variant_a_star_not_naive = lambda t, pt: calculate_optimal_suffix_alignment(
            t, pt, naive=False, use_dijkstra=False
        )

        return [
            variant_dijkstra_naive,
            variant_dijkstra_not_naive,
            variant_a_star_naive,
            variant_a_star_not_naive,
        ]
