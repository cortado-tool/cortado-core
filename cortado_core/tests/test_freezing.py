...
class TestFreezing(unittest.TestCase):
    def test_frozen_subtree_removal_when_corresponding_flag_is_true(self):
        ...
        resulting_pt, frozen_subtrees = add_trace_to_pt_language_with_freezing(
            pt,
            [frozen_subtree],
            EventLog(),
            trace,
    ...
    def test_frozen_subtree_removal_when_corresponding_flag_is_true_multiple_missing_subtrees(
        self,
    ):
        ...
        resulting_pt, res_frozen_subtrees = add_trace_to_pt_language_with_freezing(
            pt,
            frozen_subtrees,
            EventLog(),
            trace,
        ...
        self.assertEqual(
            parse("+(->('a', 'd'), X(tau,->('e', 'f', 'g')), X(tau,X('b', 'c')))"),
            resulting_pt,
        )
        self.assertEqual(frozen_subtrees, res_frozen_subtrees)

    def test_frozen_subtree_removal_when_corresponding_flag_is_false_multiple_missing_subtrees(
        self,
    ):
        pt = parse("->('a', X('b', 'c'), ->('e', 'f', 'g'), 'd')")
        frozen_subtrees = [pt.children[1], pt.children[2]]
