...
def convertParseTreeToQueryTree(
    context: ParserRuleContext, negated: bool = False
) -> QueryTree:
    qTree = None

    if isinstance(context, vqlParser.CnfClauseContext) or isinstance(
        context, vqlParser.DnfClauseContext
    ):
        children = []

        lOp = (
            LOperator.OR
            if isinstance(context, vqlParser.DnfClauseContext)
            else LOperator.AND
        )
    ...
    elif isinstance(context, vqlParser.LogicBlockContext) or isinstance(
        context, vqlParser.QueryContext
    ):
