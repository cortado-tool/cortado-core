...
def fix_concurrency_pairs_with_self_concurrency(concurreny_pairs):
    for (activity1, activity2), indices in concurreny_pairs.items():
        if activity1 == activity2:
            indices.update({(indice2, indice1) for (indice1, indice2) in indices})
...
def check_quantified_concurrent(
    ...
    if len(rActivities) > 1:
        ...
        if any_activity:
            ...
            # Compute the length of the set of lActivites followed by All of the rActivities
            return check_quantified_operator(
                len(set.intersection(*lIds)), quantifier, occ
            )
...
def check_concurrent(
    ...
    fix_concurrency_pairs_with_self_concurrency(graph_elements)
