...
from cortado_core.eventually_follows_pattern_mining.util.tree import (
    ...
    get_right_check_node_for_full_occurrence,
)
...
class FullOccurrenceStore:
    ...
    def update_occurrence_lists_for_initial_pattern_with_operator(
        ...
        for i, tree in enumerate(self.trees):
            nodes = get_nodes_with_operator(operator, tree)
            occurrence_list = [[node] for node in nodes]
    ...
    def update_occurrence_lists_for_initial_pattern_with_label(
        ...
        for i, tree in enumerate(self.trees):
            nodes = get_leaf_nodes_with_label(label, tree)
            occurrence_list = [[node] for node in nodes]
            self.update_occurrence_list_for_pattern_and_tree(
                i, occurrence_list, pattern
            )
    ...
    def set_support_for_initial_pattern(self, pattern: EventuallyFollowsPattern):
        ...
        pattern.support = (
            self.counting_strategy.get_support_for_1_pattern_full_occ_list(
    ...
    def update_occurrence_lists_using_predecessor_pattern_for_new_subpattern(
        ...
        for tree_id in self.occurrence_lists[predecessor_pattern.id]:
            ...
            for matching_occurrences in matching_frequent_last_sub_pattern_occ_list:
                right_check_node = matching_occurrences[0]
                for predecessor_occurrences in predecessor_occurrence_list:
                    left_check_node = get_left_check_node_for_full_occurrence(
                        predecessor_pattern.sub_patterns[-1], predecessor_occurrences
                    )
                    if is_eventually_follows_relation_with_ef_dict(
                        left_check_node, right_check_node, self.ef_dict[tree_id]
                    ):
                        occurrence = predecessor_occurrences.copy()
                        occurrence.append(matching_occurrences[0])
    ...
    def update_occurrence_lists_for_sub_pattern_with_single_child_and_sequential_root(
        ...
        for tree_id in self.occurrence_lists[predecessor_pattern.id]:
            ...
            for predecessor_occurrences in predecessor_occurrence_list:
                if predecessor_occurrences[-1].parent is None:
                    continue

                if (
                    predecessor_occurrences[-1].parent.op
                    != ConcurrencyTreeOperator.Sequential
                ):
                    continue

                occurrences = copy.copy(predecessor_occurrences)
                occurrences[-1] = predecessor_occurrences[-1].parent
                occurrences.append(predecessor_occurrences[-1])
                new_occurrence_list.append(occurrences)
        ...
        for tree_id in self.occurrence_lists[predecessor_pattern.id]:
            ...
            for predecessor_occurrences in predecessor_occurrence_list:
                search_node = get_first_search_node(
                    pattern.height_diff, predecessor_occurrences[-1]
                )
                ...
                for rm_occurrence in rm_occurrence_list:
                    occurrences = copy.copy(predecessor_occurrences)
                    occurrences.append(rm_occurrence)
                    new_occurrence_list.append(occurrences)
    ...
    def update_support_to_gain(
        ...
        return (
            support_to_gain
            - self.counting_strategy.get_support_for_single_tree_full_occ_list(
                predecessor_occurrence_list, tree_id, predecessor_pattern
            )
        )
