...
class TraceOccurrenceCountingStrategy(CountingStrategy):
    ...
    def get_support_for_1_pattern(
        ...
        for tree_id, occ in occurrences.items():
            ...
            support += len(rmo_ids) * self.trees[tree_id].n_traces
    ...
    def get_support_for_single_tree(
        ...
        return support * self.trees[tree_id].n_traces
    ...
    def get_support_for_1_pattern_combination_occ_list(
        ...
        for tree_id, occ in occurrences.items():
            ...
            support += len(root_ids) * self.trees[tree_id].n_traces
    ...
    def get_support_for_1_pattern_full_occ_list(
        ...
        for tree_id, occ in occurrences.items():
            ...
            support += len(ids) * self.trees[tree_id].n_traces
    ...
    def get_support_for_single_tree_full_occ_list(
        ...
        for nodes in occurrence_list:
            for i, sub_pattern_root in enumerate(pattern.sub_patterns):
                distinct_roots_for_sub_patters[i].add(nodes[sub_pattern_root.id].id)
